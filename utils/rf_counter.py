...
class RainFlowCounter:
    def __init__(self, file_path,
        ...
        if verbose:
            logger.info(f'\t▪ Data successfully loaded in '
        ...
        if racetrack_filter:
            self.nz, ix = \
                fatpack.find_reversals_racetrack_filtered(self.nz, h=h, k=200)
            self.tt = self.tt[ix]
        ...
        if verbose:
            logger.info(f'\t▪ Load cycles successfully extracted in '
                        f'{round((datetime.now() - t0).total_seconds(), 3)} '
                        f's.')
    ...
    def __init__(self, verbose=False):
        ...
        for file_name in imu_files:
            ...
            if ext != '.dat':
                ...
                logger.info(f'• Analyzing flight "{file_name.split(".")[0]}"')
